...
GTM Dashboard is a web app that brings GTM data (Google Analytics 4, Google Search Console, Google Ads, HubSpot) into **BigQuery** for each customer (tenant). Users sign up and authenticate with **Supabase**, pay with **Stripe**, connect their tools, ask questions in plain English, and get charts + short insights. We keep the architecture **lean**: one API service on **Cloud Run**, **Supabase Postgres** for app records, **BigQuery** for analytics tables and **materialized views**. No dbt, no Pub/Sub/Workflows/Scheduler/Jobs unless we hit a wall.
...
* **Tenant Isolation:** One BigQuery dataset per tenant (`tenant_{id}`); permissions scoped.
* **Integrations (no Airbyte):**

  * GA4, Google Ads, Google Search Console via **BigQuery native transfers/exports** (provider-managed scheduling; we don’t run schedulers).
  * HubSpot via **simple backend ingestion** (our API calls HubSpot REST and writes to BigQuery) to avoid GCS buckets and extra infra.
* **Analytics Engine:** Google **ADK** agent service on Cloud Run (schema read, SQL generation, DRY-RUN/cost cap, execution).
...
* **BigQuery**: source tables per integration; **views** and **materialized views** for curated analytics; **SELECT-only** access for the agent.
* **Stripe**: hosted Checkout + Billing Portal; single webhook endpoint in API.

> Why no GCS bucket?
>
> * GA4, GSC, and Google Ads can land **directly** in BigQuery through native exports/transfers.
> * For HubSpot, to **avoid GCS**, we will **pull via API** from our backend and write directly to BigQuery tables. If later we adopt HubSpot’s native BQ integration and it requires GCS as a staging step, we will introduce a minimal bucket then—but **not** for this MVP.
...
* Raw/landing tables: `ga4_*`, `gsc_*`, `google_ads_*`, `hubspot_contacts`, `hubspot_deals`, `hubspot_engagements` (names kept simple).
...
* `users` (Supabase-managed) + `tenant_memberships` (optional; can keep one owner)
...
## 7) APIs Needed (describe, not endpoints)

**Belongs to the single API service on Cloud Run**

1. **Auth & Tenant**

   * Verify Supabase JWT; resolve current tenant.
   * Provision tenant: create BigQuery dataset, store record in Supabase.
...
3. **Integrations**
   ...
   * Connect GA4 / Google Ads / GSC: create native BigQuery transfer/export into the tenant dataset (provider-managed schedule).
   * Connect HubSpot: store OAuth/token (in Supabase **encrypted**) and save config.
   * Manual refresh (per source): run a **one-shot** fetch (HubSpot only; GA4/Ads/GSC rely on provider-managed updates).
...
> Note: Keeping one service reduces complexity and team handoffs. If we later split, these become microservice boundaries.
...
4. **Connect sources**
   ...
   * GA4/Ads/GSC: finish provider UIs; data starts to land in BigQuery (no in-house scheduler).
...
7. **Manual refresh** when needed (HubSpot); GA4/Ads/GSC refresh on provider cadence.
...
* **Performance**
  ...
  * BigQuery handles large scans efficiently by default; we add **DRY-RUN + max bytes** to avoid surprises.
...
* **Scalability**
  ...
  * Cloud Run autoscales the single API service by default; no tuning needed for early usage.
  * BigQuery scales transparently; each tenant has its own dataset to avoid cross-tenant contention.
  * Provider-managed transfers (GA4/Ads/GSC) remove our need to run schedulers.
...
* GA4 or GSC or Ads connected and visible in the Integrations screen with **last sync** info.
* HubSpot “Connect” works; **Manual Sync** ingests contacts and deals into BigQuery.
...
* GA4, GSC, and Google Ads “native to BigQuery” transfers are set up per tenant; they are **provider-managed schedules** (we do not run a scheduler).
...
* **API/Backend:** Single Cloud Run service (Gateway + Integrations + Agent + Charts + Billing); BigQuery DDL (tables, views, materialized views); HubSpot pull logic.
* **Data Admin:** Create/maintain allow-listed views & materialized views per tenant (SQL files in repo).
* **Ops (shared):** Supabase project, Stripe product/price, Cloud Run service, BigQuery IAM templates.
